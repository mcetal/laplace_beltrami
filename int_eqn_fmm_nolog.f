...
c
c  This code resolves the multiply connected case by allowing different
c  constant values of the streamline on the boundaries and zero
c  circulation around each contour
c
c
c     ------------------------------------------------------------------
      implicit real*8 (a-h, o-z)
      parameter (kmax = 100, npmax = 5000, nmax = kmax*npmax)
c
c Geometry of holes
...
c
         call SOLVE (nd, k, nbk, rhs, soln, density, A_k, gmwork, 
...
ccc         rhs(nbk+1) = gamma_tot
         do kbod = 2, k
            rhs(nbk+kbod-1) = 0.d0
         end do
         call PRIN2 (' rhs at end = *', rhs(nbk+1), k-1)
...
c********1*********2*********3*********4*********5*********6*********7**
c
      subroutine MATVEC_SPHERE (k, nd, nbk, qa, xs, ys, zs, xn, yn, zn,
     1                          dsda, diag, aKu)
c---------------
      implicit real*8 (a-h,o-z)
      dimension qa(nbk), xs(nbk), ys(nbk), zs(nbk), xn(nbk), yn(nbk),
     1          zn(nbk), diag(nbk), aKu(nbk), dsda(nbk)
      dimension gradu(3), vn(3), R0(3), R(3), delR(3)
c
         pi = 4.d0*datan(1.d0)
         dalph = 2.d0*pi/nd
c
         call PRIn2 (' qa = *', qa, nbk)
         do i = 1, nbk
            aKu(i) = 0.d0
            R0(1) = xs(i)
            R0(2) = ys(i)
            ...
            do j = 1, i-1
               ...
               call DOT (delR, vn, du_dn)
                du_dn = du_dn/rad2
                du = dalph*qa(j)*du_dn*dsda(j)/(2.d0*pi)
ccc               aKu(i) = aKu(i) - dalph*qa(j)*du_dn*dsda(j)/(2.d0*pi)
               aKu(i) = aKu(i) +
     1                    dalph*qa(j)*du_dn*dsda(j)/(2.d0*pi)
            end do
            do j = i+1, nbk
               vn(1) = xn(j)
               vn(2) = yn(j)
...
c********1*********2*********3*********4*********5*********6*********7**
c
      subroutine  FASMVP (k, nd, nbk, nsp, x_zeta, y_zeta, zeta, dzeta,   
     1                    zeta_k, diag, dsda, A_k, u, w, qa, cfield,  
     2                    poten, wksp)
c---------------
c
      implicit real*8 (a-h, o-z)
...
c
c Constraints for a single log but with different-valued streamlines
         istart = nd
         do kbod = 2, k
            do i = 1, nd
               w(istart+i) = w(istart+i) + A_k(kbod)
            end do
            istart = istart + nd
         end do
         do kbod = 2, k
            ...
            do i = (kbod-1)*nd+1, kbod*nd
               w(nbk + kbod-1) = w(nbk + kbod-1) + dalph*u(i)
            end do
         end do 
         tend = etime(timep)
...
c
c---------------
      subroutine SOLVE (nd, k, nbk, rhs, soln, u, A_k, rwork, 
...
ccc            call PRIn2 (' density = *', u, nbk)
            do kbod = 2, k
ccc               call PRIn2 (' in solve, u = *', u((kbod-1)*nd+1), nd)
...
c
     ...
     1                   - dalph*u(ip)*dimag(zkern)/(2.d0*pi)
               ...
               do kbod = 1, k
                  ...
                  rad = 
     1              4.d0*(cdabs(zdis))**2/((1+(cdabs(zeta_gr(i,j)))**2)
...
c
c---------------
      subroutine MSOLVE (N, XX, YY, NELT, IA, JA, A, ISYM)
c---------------
c
      return
      end
...
c
     ...
     1                zeta_k, diag, dsda, A_k, xx, yy, qa, cfield,  
     2                poten, wksp)
