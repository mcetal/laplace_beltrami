      PROGRAM MULT_CONN_TEST
c     ------------------------------------------------------------------
c
c
c  tests some multiply-connected issues
c
c
c     ------------------------------------------------------------------
...
c
         do kbod = 1, k
            ...
            call PRIN2 (' U = *', soln, nbk)
            call MATVEC_LAPL (nbk, soln, rhs, NELT, IA, JA, A, ISYM)
            do lbod = 1, k
               ist = (lbod-1)*nd+1
               call PRIN2 (' KU = *', rhs(ist), nd)
            end do
         end do
c
      stop
...
c Fix up field
     ...
     1                   /(1.d0+cdabs(zeta(i))**2)
            ...
            cfield(i) = cfield(i) + zQ2sum
            w(i) = 0.5d0*u(i) + dimag(cfield(i)) - dalph*diag(i)*u(i)
         end do
ccc         call PRIN2 (' w 1 = *', w, nbk)
            do lbod = 1, k
               ist = (lbod-1)*nd+1
               call PRIN2 (' w = *', w(ist), nd)
            end do
            do i = 1, nbk
               w(i) = w(i) - dimag(zQsum)
            end do
         call PRIN2 (' w 2 = *', w, nbk)
